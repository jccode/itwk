	...
	public function add_auth($data,$file_name = '',$is_jump=true,$file_name_back = '') {
		...
		if (! $id_pic || ! $id_pic_back || ! $data ['id_card'] || ! $data ['realname']) {
			kekezu::show_msg ( "操作提示", $_SERVER['HTTP_REFERER'], '1', $this->auth_lang().$_lang['apply_submit_fail_for_info_little'], 'alert_error' ) ;
			 ...
			 //判断身份证号码是否已存在 
			$already_id_card = db_factory::get_count(" select count(*) from " . TABLEPRE . "witkey_auth_realname where id_card='$data[id_card]' and uid <> '$user_info[uid]'");
			...
			$id_starttime and $data['id_starttime'] = $id_starttime;
			$id_address and $data['id_address'] = $id_address;
			$id_type and $data['id_type'] = $id_type;
