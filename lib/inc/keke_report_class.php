...
abstract class keke_report_class {
	...
	public static function accept_notify($report_info, $user_info, $re_obj) {
		...
		$result = array ("任务编号"=>$report_info['origin_id'],"处理结果"=>$report_info['op_result']);
	 ...
	 * 
	 */
	public static function add_report($obj, $obj_id, $to_uid, $to_username, $desc, $report_type, $report_cate, $front_status = null, $origin_id = null, $user_type = null, $file_info = NULL, $is_hide = 1) {
		global $uid, $username;
		global $_lang;
		...
		$report_obj->setReport_cate( $report_cate );
		$report_obj->setFront_status ( $front_status );
		$report_obj->setReport_file ( $file_info['save_name'] );
		...
		if ($report_id) {
			...
			if($file_info['file_id']){
				...
				$file_obj->setWhere("file_id=".intval($file_info['file_id']));
				$file_obj->setObj_id($report_id);
	...
	public function cancel_bid($obj_id, $trust_response = false) {
		...
		if ($this->_obj_info ['is_trust'] && $trust_response == false&&$this->_process_can['cancel_bid']) { //，非单人模式才有取消中标,担保定向
			$request_url = keke_trust_fac_class::trust_task_request ( "pt_cancel", $model_info ['model_dir'], $this->_obj_info ['origin_id'], $this->_obj_info ['trust_type'], $obj_id );
			$_GET  = array();$_POST = array();
			$_GET['resultMsg'] = file_get_contents($request_url);
			if($_GET){
				require_once S_ROOT.'/payment/'.$this->_obj_info['trust_type'].'/lib/alipay_notify.class.php';
				$_GET = AlipayNotify::data_merge(strtoupper(CHARSET));
				$class = $model_info['model_dir']."_".$this->_obj_info['trust_type']."_class";
				$fac_obj = new $class ($this->_obj_info['origin_id'],'pt_cancel',$_GET);
				$res = $fac_obj->pt_cancel( true);
			}
			die ();
		} else {
	...
	public static function get_transrights_name($report_type = null) {
		global $_lang;
		$type_arr = array ("1" => $_lang['report'], "2" => $_lang['complaints'] );
		if ($report_type) {
	...
	public static function get_transrights_type() {
		global $_lang;
		return array ("report" => array ("1", $_lang['report'] ), "complaint" => array ("2", $_lang['complaints'] ) );
	}
	/**
	 * 获取交易维权对象
	 */
	public static function get_transrights_obj($obj = null) {
		global $_lang;
		$trans_obj = array ("task" => $_lang['task'], "work" => $_lang['work'], "user" => "用户");
		if ($obj){
	...
	public static function get_transrights_status() {
		global $_lang;
		return array ("1" => $_lang['wait_process'], "2" => $_lang['has_process'] );
	}
	/**
	 * 扣除信誉，与能力值
	 * @param int $value  值
