...
class keke_agent_class {
	public $_uid;
	public function __construct() {
		$_SESSION ['uid'] and $this->_uid = $_SESSION ['uid'];
	}
	static function get_admin_menu() {
		global $kekezu, $_lang, $admin_info;
		$menuset_arr = $kekezu->_cache_obj->get ( 'agent_menu_resource_cache' );
		if (! $menuset_arr) {
			...
			foreach ( $resource_arr as $r_tp ) {
				$grouplist_arr = keke_agent_class::get_user_group ();
				if (is_string ( $_SESSION ['group_id'] )) { // 如果为非代理商进入，则显示所有代理商菜单
					$group_arr = explode ( ',', $_SESSION ['group_id'] );
				} else {
					$group_arr = array (
							$_SESSION ['group_id'] 
					);
				...
				foreach ( $group_arr as $v ) {
					if (! $grouplist_arr [$v] ['group_roles']) { // 不是从登录窗口过来的
						echo "<script>window.parent.location.href='index.php?do=login';</script>";
						die ();
					}
					if (in_array ( $r_tp ['resource_id'], $grouplist_arr [$v] ['group_roles'] )) { // 把代理商有的菜单显示出来
			...
			$menuset_arr = array (
					'navgat' => $navgation_arr,
					'menu' => $resource_arr,
					'submenu' => $submenu_set_arr,
					'resource' => $resource_set_arr 
			);
			$kekezu->_cache_obj->set ( 'agent_menu_resource_cache', $menuset_arr, null );
		}
		return $menuset_arr;
	}
	static function get_user_group() {
		global $kekezu;
		$group_arr = $kekezu->_cache_obj->get ( "agent_member_group_cache" );
		if (! $group_arr) {
			...
			$group_arr = $temp_arr;
			$kekezu->_cache_obj->set ( 'agent_member_group_cache', $group_arr, null );
		}
	...
	public function get_shortcuts_list() {
		$sql = " select b.resource_id,b.resource_name,b.resource_url from " . TABLEPRE . "witkey_shortcuts a left join " . TABLEPRE . "witkey_resource b on a.resource_id = b.resource_id where FIND_IN_SET(b.resource_id,(select group_roles from " . TABLEPRE . "witkey_member_group where group_id in(" . $_SESSION ['group_id'] . "))) and a.uid = '$this->_uid' order by a.s_id desc ";
	...
	public function admin_login($username, $password, $allow_times, $formhash = '') {
		...
		if ($login_limit && $remain_times > 0) { // 存在登录时间限制并且时限未过
			...
			if (! kekezu::submitcheck ( $formhash, true )) { // 检测hash值
				...
				$kekezu->echojson ( $_lang ['repeat_form_submit'], 6, array (
						'times' => $allow_times,
						'formhash' => $hash 
				) );
				die ();
			...
			if ($user_info == - 1) {
				$_SESSION ['allow_times'] -= 1;
				-- $allow_times == 0 and $this->set_login_limit_time ( '1' );
				$kekezu->echojson ( $_lang ['username_input_error'], "6", array (
						'times' => $allow_times,
						'formhash' => $hash 
				) );
				die ();
				...
				$_SESSION ['allow_times'] -= 1;
				-- $allow_times == 0 and $this->set_login_limit_time ( '1' );
				$kekezu->echojson ( $_lang ['password_input_error'], "5", array (
						'times' => $allow_times,
						'formhash' => $hash 
				) );
				die ();
			...
			if (! $user_info) {
				$_SESSION ['allow_times'] -= 1;
				-- $allow_times == 0 and $this->set_login_limit_time ( '1' );
				$kekezu->echojson ( $_lang ['login_fail'], "4", array (
						'times' => $allow_times,
						'formhash' => $hash 
				) );
				die ();
			...
			if (! $user_info) {
				$_SESSION ['allow_times'] -= 1;
				-- $allow_times == 0 and $this->set_login_limit_time ( '1' );
				$kekezu->echojson ( $_lang ['no_rights_login_backstage'], "3", array (
						'times' => $allow_times,
						'formhash' => $hash 
				) );
				die ();
				...
				$_SESSION ['allow_times'] -= 1;
				-- $allow_times == 0 and $this->set_login_limit_time ( '1' );
				$kekezu->echojson ( $_lang ['no_rights_login_backstage'], "2", array (
						'times' => $allow_times,
						'formhash' => $hash 
				) );
				die ();
				...
				if ($user_info ['group_id'] != 12) { // 如果为总管理员，则显示组为所有代理商组的组合
					$_SESSION ['group_id'] = "12";
				} else {
					$_SESSION ['group_id'] = "" . $user_info ['group_id'];
				}
				switch ($user_info ['group_id']) {
					case '12' :
						$_SESSION ['brandType'] = "'tw'";
						break;
					default :
						$_SESSION ['brandType'] = "'tw','hk'";
						break;
				}
	...
	public static function daily_statistics($day = 0) {
		...
		$submit ['reward'] = array (
				'count' => intval ( $t_r_c ['count'] ),
				'cash' => floatval ( $t_r_c ['cash'] ) 
		);
		$submit ['preward'] = array (
				'count' => intval ( $t_p_c ['count'] ),
				'cash' => floatval ( $t_p_c ['cash'] ) 
		);
		$submit ['tender'] = array (
				'count' => intval ( $t_t_c ['count'] ),
				'cash' => floatval ( $t_t_c ['cash'] ) 
		);
		$submit ['service'] = array (
				'count' => intval ( $t_s_c ['count'] ),
				'cash' => floatval ( $t_s_c ['cash'] ) 
		);
		$submit ['hire'] = array (
				'count' => intval ( $t_h_c ['count'] ),
				'cash' => floatval ( $t_h_c ['cash'] ) 
		);
		$submit ['taskhire'] = array (
				'count' => intval ( $t_th_c ['count'] ),
				'cash' => floatval ( $t_th_c ['cash'] ) 
		);
		// 总计
		$submit ['total'] = array (
				'count' => intval ( $t_r_c ['count'] + $t_p_c ['count'] + $t_t_c ['count'] + $t_s_c ['count'] + $t_h_c ['count'] + $t_th_c ['count'] ),
		...
		$success ['reward'] = array (
				'count' => intval ( $t_r_s ['count'] ),
				'cash' => floatval ( $t_r_s ['cash'] ),
				'host' => floatval ( $m_r_h ['cash'] ) 
		);
		$success ['preward'] = array (
				'count' => intval ( $t_p_s ['count'] ),
				'cash' => floatval ( $t_p_s ['cash'] ),
				'host' => floatval ( $m_p_h ['cash'] ) 
		);
		$success ['tender'] = array (
				'count' => intval ( $t_t_s ['count'] ),
				'cash' => floatval ( $t_t_s ['cash'] ),
				'host' => floatval ( $m_t_h ['cash'] ) 
		);
		$success ['service'] = array (
				'count' => intval ( $t_s_s ['count'] ),
				'cash' => floatval ( $t_s_s ['cash'] ),
				'host' => floatval ( $m_s_h ['cash'] ) 
		);
		$success ['hire'] = array (
				'count' => intval ( $t_h_s ['count'] ),
				'cash' => floatval ( $t_h_s ['cash'] ),
				'host' => floatval ( $m_h_h ['cash'] ) 
		);
		$success ['taskhire'] = array (
				'count' => intval ( $t_th_s_g ['count'] ),
				'cash' => floatval ( $t_th_s ['cash'] ),
				'host' => floatval ( $m_th_h ['cash'] ) 
		);
		// 总计
		$success ['total'] = array (
				'count' => intval ( $t_r_s ['count'] + $t_p_s ['count'] + $t_t_s ['count'] + $t_s_s ['count'] + $t_h_s ['count'] + $t_th_s_g ['count'] ),
		...
		$complete ['reward'] = array (
				'count' => intval ( $t_r_e ['count'] ),
				'cash' => floatval ( $t_r_e ['cash'] ) 
		);
		$complete ['preward'] = array (
				'count' => intval ( $t_p_e ['count'] ),
				'cash' => floatval ( $t_p_e ['cash'] ) 
		);
		$complete ['tender'] = array (
				'count' => intval ( $t_t_e ['count'] ),
				'cash' => floatval ( $t_t_e ['cash'] ) 
		);
		$complete ['service'] = array (
				'count' => intval ( $t_s_e ['count'] ),
				'cash' => floatval ( $t_s_e ['cash'] ) 
		);
		$complete ['hire'] = array (
				'count' => intval ( $t_h_e ['count'] ),
				'cash' => floatval ( $t_h_e ['cash'] ) 
		);
		$complete ['taskhire'] = array (
				'count' => intval ( $t_th_e ['count'] ),
				'cash' => floatval ( $t_th_e ['cash'] ) 
		);
		// 总计
		$complete ['total'] = array (
				'count' => intval ( $t_r_e ['count'] + $t_p_e ['count'] + $t_t_e ['count'] + $t_s_e ['count'] + $t_h_e ['count'] + $t_th_e ['count'] ),
		...
		$additional ['register'] = array (
				'count' => intval ( $today_register_count ),
		...
		$additional ['vipbuy'] = array (
				'new_count' => intval ( $today_vipbuy_count ['count'] ),
		...
		$additional ['oatuh'] = array (
				'count' => intval ( $today_oauth_count ),
		...
		$additional ['mail'] = array (
				'count' => intval ( $today_mail_count ),
		...
		if ($id) { // 更新
			db_factory::updatetable ( TABLEPRE . 'witkey_statistics', array (
					'submit' => serialize ( $submit ),
			...
			$id = db_factory::inserttable ( TABLEPRE . 'witkey_statistics', array (
					'submit' => serialize ( $submit ),
					'success' => serialize ( $success ),
					'conversion' => serialize ( $conversion ),
					'complete' => serialize ( $complete ),
	 ...
	 * @param string $table        	
	 * @param string $where        	
	 * @param string $auth_field判断权限的字段，默认为brand,即是以brand为权限判断条件        	
	 *        	当$auth_field不为brand时，判断其它依据所根据的品牌馆类型标准，如台湾馆时手机为09开头，身份证为a开头等，此时$brand='tw'
	 * @return boolean true有权限，false无权限
	 */
	function agent_auth($table, $where, $auth_field = 'brand') {
		$rs = db_factory::get_one ( ' select uid,'.$auth_field.' from ' . TABLEPRE . $table . ' where ' . $where );
		if(!$rs||$rs[$auth_field]=='') return false;
		$auth_field_val=$rs[$auth_field];
		switch ($auth_field) {
			case 'id_card' :
				//台湾则按台湾的身份证号格式来判断
				strpos($_SESSION['brandType'], 'tw')!==false&&preg_match('/^[aA]{1}\d{9}$/', $auth_field_val)!==false and $result=true;
				break;
			case 'mobile' :
				//台湾则按台湾的手机号格式来判断
				strpos($_SESSION['brandType'], 'tw')!==false&&(preg_match('/^09\d{8}$/', $auth_field_val)||preg_match('/^(00886)?[-| ]?0?9\d{8}$/', $auth_field_val)) and $result=true;
				break;
			case 'deposit_area':
				//银行卡发卡点
				strpos($_SESSION['brandType'], 'tw')!==false and strpos($auth_field_val, '台湾')!==false and $result=true;	
			    break;
			default :
			case 'shop_level'://vip商铺管理
			case 'email_a_id':
			case 'service_id':
			case 'case_id':
				$rs = db_factory::get_one ( 'select brand from  '. TABLEPRE .'witkey_space where uid='. $rs['uid']);
				if(!$rs) return false;
				strpos($_SESSION['brandType'], $rs['brand'])!==false and $result=true;
				break;
			case 'brand' :
				if (strpos ( $_SESSION ['brandType'], $auth_field_val ) !== false) {
					// 如果管理员session['brand']中含有记录相应的brand值
					$result = true;
				} else {
					$result = false;
				}
