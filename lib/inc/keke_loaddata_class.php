...
class keke_loaddata_class {
	...
	//显示广告的预览
	static function preview_addgroup($adname) {
		self::adgroup ( $adname );
	}
	...
	static function ad_show($code, $do = 'index',$is_slide=null) {
		...
		if ($ad_target) {
			if($is_slide==1){
			    return self::adgroup($ad_target['target_id'],$ad_target ['ad_num']);
			}
				...
				if ($ad_list [$k]) {
					...
					switch ($ad_info ['ad_type']) {
						...
						case "code" :
							...
							break;
						case "image" :
							$ad_str .= "<a href='" . $ad_info ['ad_url'] . "' target='_blank'><img src='" . $_K [siteurl] . '/' . $ad_info ['ad_file']
									 . "' width='".$ad_info ['width']."' height='".$ad_info ['height']."' alt='".$ad_info['ad_name']."'></a>";
							break;
					}
					self::update_ad($ad_info);
				} else {
					$ad_str .= "<div class='adv w" . $pos [0] . "xh" . $pos [1] . " ad_" . $k . "'>";
					$ad_str .= "<a href='' target='_blank'><p class='t_c'>".$_lang['ad_leasing']."</p><p class='t_c'>" . $pos [0] . "x" . $pos [1] . $_lang['pixels']. "  </p></a>";
				}
				$ad_str .= "</div>";
	...
	static function ad($adid) {
		...
		if ($ad_info ['ad_type'] == 1) {
			$adstr = '<embed src="' . $_K [siteurl] . '/' . $ad_info ['ad_file'] . '" quality="high" width="' . $ad_info ['width'] . '" height="' . $ad_info ['height'] . '" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash"></embed>';
		} elseif ($ad_info ['ad_type'] == 3) {
			$adstr = htmlspecialchars_decode ( $ad_info ['ad_content'] );
		} else {
			$adstr = '<img alt="'.$ad_info['ad_name'].'" src="' . $_K [siteurl] . '/' . $ad_info ['ad_file'] . '" ';
	...
	static function adgroup($adname,$ad_limit_num) {
		global $kekezu;
		$kekezu->_tag or $kekezu->init_tag();
		$ad_target = db_factory::get_one ( sprintf ( " select * from %switkey_ad_target where target_id='%s' and is_allow=1", TABLEPRE, $adname),3600*24);
		$datalist = kekezu::get_ad ( $adname,$ad_limit_num );
		$tag_arr = $kekezu->_tag;
		$tag_info = $tag_arr [$ad_target['name']];
		require keke_tpl_class::parse_code ( htmlspecialchars_decode ( $tag_info ['tag_code'] ), $tag_info ['tag_id'] );
	}
