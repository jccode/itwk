...
abstract class keke_privission_class {
	 ...
	 * @return boolean
	 */
	public static function check_condit_priv($uid, $priv,$model_name,$notice) {
		global $_lang;
			...
			if (empty($condition)) { //没有设置条件
				...
				foreach ( $condition as $k=>$v ) {
					if ($item_list [$v] && $item_list [$v] ['auth_open'] == '1') { //当前有此认证并开启
						if (! keke_auth_fac_class::auth_check ( $v, $uid )) {
	...
	public static function check_priv($task_id,$user_info, $priv,$model_name,$notice) {
		...
		if ($user_info) {
			switch ($user_info [uid] === ADMIN_UID || $user_info ['group_id'] === 7) { //是否管理员.客服
				...
				case "0" : //不是管理员，老实接受审核
					/*-----条件限制----*/
					$condit_return = self::check_condit_priv ( $user_info ['uid'], $priv,$model_name,$notice);
					/*-----权限限制----*/
