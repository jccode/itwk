...
class keke_shop_class {
	...
	public static function get_contact($uid) {
	 ...
	 /**
	 * 更新商铺被浏览记录
	 * @param int $shop_id 商铺编号
	 * @param int $v_uid 商铺UID
	 * @param int $v_uid 浏览者UID
	 * @param int $v_username 浏览者用户名
	 */
	public static function shop_visit($shop_id, $uid, $v_uid, $v_username){
		if( $uid == $v_uid ) return false;
		if( !$shop_id || !$uid || !$v_uid || !$v_username ) return false;
	
		$visit_info = db_factory::get_one(" select * from " . TABLEPRE . "witkey_shop_visit where shop_id='$shop_id' and uid='$v_uid'");
		
		if( $visit_info ){ //有则更新浏览时间
			db_factory::execute ( " update " . TABLEPRE . "witkey_shop_visit set on_time = '" . time() . "' where v_id = '$visit_info[v_id]'" );
		}else{ //没有记录则添加
			db_factory::execute ( " insert into " . TABLEPRE . "witkey_shop_visit (`shop_id`, `uid`, `username`, `on_time`) values ('$shop_id', '$v_uid', '$v_username', '" . time() . "')" );	
		}
	}
	...
	public static function create_service_order($service_info) {
		...
		if ($order_id) {
			...
			$feed_arr = array ("feed_username" => array ("content" => $username, "url" =>"index.php?do=shop&u_id=".$uid), "action" => array ("content" => $_lang['buy'], "url" => '' ), "event" => array ("content" => $order_name, "url" =>"index.php?do=service&sid=$service_info[service_id]" ) );
   ...
   public static function get_mark_count_ext($model_code, $sid){
	 ...
	 * 检查个性域名是否存在
	 *  */
	public static function check_domain($check_domain){
		$res = kekezu::get_table_data ( "shop_id", "witkey_shop", " domain = '$check_domain' ", "", "", "", "" );
		if($res){
			return "个性域名已被占用";
		}else{
			return true;
	 ...
	 * 根据域名取sid
	*  */
	public static function get_shop_by_domain(){
		$domain = array();

		// 二级目录 /字母+数字/ (自定义), /数字/ (sid)
		if ( preg_match('/^shop.epweike(\.(com|net))?(\.cn)?$/i', $_SERVER['HTTP_HOST']) ) {
			preg_match('/^\/([^\/]+)/i', $_SERVER['REQUEST_URI'], $matchs);

			if ( is_numeric( $matchs[1] ) ) {
				$domain['sid'] = $matchs[1];
				$domain['type'] = 2;
				$domain['status'] = 1;
			} else {
				$sql = "SELECT * FROM " . TABLEPRE . "witkey_shop_domain WHERE d_key = '" . $matchs[1] . "' AND d_type = 0";
			}
		}
		// 二级域 http://***.itbangshou.com/
		elseif ( preg_match('/^(\w+)\.epweike(\.(com|net))?(\.cn)?$/i', $_SERVER['HTTP_HOST'], $matchs) ) {
			$sql = "SELECT * FROM " . TABLEPRE . "witkey_shop_domain WHERE d_key = '" . $matchs[1] . "' AND d_type = 1";
		}
		// 顶级域名
		else {
			$sql = "SELECT * FROM " . TABLEPRE . "witkey_shop_domain WHERE d_key = '" . $_SERVER['HTTP_HOST'] . "' AND d_type = 2";
		}

		if ( ! $domain && $sql ) {
			if ( $re = db_factory::get_one( $sql ) ) {
				$domain['sid'] = $re['shop_id'];
				$domain['type'] = $re['d_type'];
				$domain['status'] = $re['d_status'];
			}
		}
		return $domain;
	}
	 ...
	 * 检查个性域名状态
	*  */
	public static function check_domain_status(){
		global $_K;

                $shop_domain = keke_shop_class::get_shop_by_domain();

                if ( ! $shop_domain ) {
                    kekezu::show_msg ( '商铺个性域名不存在!', $_K ['siteurl'], '10', '', 'warning' );
                }elseif ( $shop_domain['status'] != 1 ) {
                    kekezu::show_msg ( '商铺个性域名审核中!', $_K ['siteurl'], '10', '', 'warning' );
                }
		return $shop_domain['sid'];
	}
}