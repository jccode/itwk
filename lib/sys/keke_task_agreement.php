...
abstract class keke_task_agreement {
	...
	public $_buyer_uid; //雇主(雇主)编号
	public $_buyer_username; //雇主(雇主)姓名
	public $_buyer_status; //雇主(雇主)状态
	public $_buyer_contact; //雇主(雇主)联系信息
	

	public $_seller_uid; //威客(威客)编号
	public $_seller_username; //威客(威客)姓名
	public $_seller_status; //威客(威客)状态
	public $_seller_contact; //威客(威客)联系信息
	 ...
	 * @param int $buyer_uid 雇主uid
	 * @param int $seller_uid 威客uid、
	...
	public function agreement_stage_one($user_type, $url = '', $output = 'normal') {
		global $_lang;
		$buyer_status = intval ( $this->_buyer_status ); //雇主签署状态
		$seller_sattus = intval ( $this->_seller_status ); //威客签署状态
			...
			switch ($user_type) {
				case "1" : //威客(威客)
					if ($seller_sattus == '2') { //签署过协议
						...
						if ($res) {
							/** 更新签署时间 */
							db_factory::execute ( sprintf ( " update %switkey_agreement set seller_accepttime='%s' where seller_uid ='%d'", TABLEPRE, time (), $this->_seller_uid ) );
							/** 雇主(雇主)签署状态判断**/
							switch ($buyer_status) {
								case "1" :
				...
				case "2" : //雇主(雇主)
					if ($buyer_status == '2') { //签署过协议
						...
						if ($res) {
							/** 更新签署时间 */
							db_factory::execute ( sprintf ( " update %switkey_agreement set buyer_accepttime='%s' where buyer_uid ='%d'", TABLEPRE, time (), $this->_buyer_uid ) );
							/** 威客(威客)签署状态判断**/
	...
	public function upfile_confirm($file_ids, $url = '', $output = 'normal') {
		...
		$res *= $this->set_agreement_status ( 'seller_status', '3' ); //变更威客为等待接收状态
		$res *= $this->set_agreement_status ( 'buyer_status', '3' ); //变更雇主为确认接收状态
	...
	public function accept_confirm($url = '', $output = 'normal', $trust_response = false) {
		...
		if ($this->_agree_status == 2 && $this->_seller_status == 3 && $this->_buyer_status == 3) {
			if ($trust_info ['is_trust'] && $trust_response == false) {
				switch ($trust_info ['is_auto_bid']) { //系统选标任务。走雇主确认中标接口
					case "1" :
						$interface = "confirm";
						$data ['work_id'] = $agree_info ['work_id'];
						$data ['is_auto_bid'] = '1';
						break;
					case "0" :
						$interface = "pt_confirm";
						$data = $agree_info ['work_id'];
						break;
				}
				$request_url = keke_trust_fac_class::trust_task_request ( $interface, $this->_model_code, $this->_task_id, $trust_info ['trust_type'], $data );
				switch ($trust_info ['is_auto_bid']) { //系统选标任务。走雇主确认中标接口
					case "1" :
						kekezu::keke_show_msg ( '', $request_url, "success", $output );
						break;
					case "0" :
						$_GET = array ();
						$_POST = array ();
						$_GET ['resultMsg'] = file_get_contents ( $request_url );
						if ($_GET) {
							require S_ROOT . '/payment/alipay_trust/lib/alipay_notify.class.php';
							$_GET = AlipayNotify::data_merge ( strtoupper ( CHARSET ) );
							$class = $this->_model_code . "_" . $trust_info ['trust_type'] . "_class";
							$fac_obj = new $class ( $this->_task_id, $interface, $_GET );
							$res = $fac_obj->$interface ( true );
							$res and kekezu::keke_show_msg ( $url, $_lang['guarantee_pay_success'], "success", $output ) or kekezu::keke_show_msg ( $url, $_lang['guarantee_pay_fail'], "error", $output );
						}
						break;
				}
