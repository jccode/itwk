...
abstract class keke_shop_release_class {
	...
	public function public_pubservice() {
		...
		/** 广告图片记录**/
		$release_info ['ad_pic_patch'] and $service_obj->setAd_pic ( $release_info ['ad_pic_patch'] );
		/** 展示图片记录**/
		...
		if ($service_id) {
			...
			switch ($service_status) {
				case "2" : //成功
					...
					$feed_arr = array ("feed_username" => array ("content" => $this->_username, "url" => "index.php?do=shop&u_id=$this->_uid" ), "action" => array ("content" => $_lang['has_pub_goods'], "url" => "" ), "event" => array ("content" => "$service_title", "url" => "index.php?do=service&sid=$service_id" ) );
	...
	public function create_payitem_reocrd($service_id, $att_info = array(), $release_info) {
		...
		if (! empty ( $att_info )) {
			...
			foreach ( $att_info as $k => $v ) {
				...
				$remain or keke_payitem_class::payitem_record ( $v ['item_code'], $v[item_num],'', 'buy', $service_id, $service_id ); //买
				//用
