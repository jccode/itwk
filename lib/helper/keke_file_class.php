...
class keke_file_class {
	...
	function get_dir_file_info($source_dir, $include_path = FALSE, $_recursion = FALSE, $sub_folder = null) {
		static $_filedata = array ();
				...
				if ($file != 'css' && $file != 'img' && $file != 'js') {
					if (strncmp ( $file, '.', 1 ) !== 0) {
						...
						if (@is_dir ( $source_dir . '/' . $file )) {
							...
							$this->get_dir_file_info ( $source_dir . '/' . $file, true, true, $file );
						}
					}
				}
			}
	...
	static function get_file_type($file_path,$ext='') {
		...
		if(in_array($typeCode,array('255216','7173','6677','13780'))){//jpg,gif,bmp,png
			in_array($ext,array('jpg','gif','bmp','png','jpeg'))&&$fileType=$ext;
		}
		if($typeCode=='208207'){//wps,ppt,dot,xls,doc,docx
			in_array($ext,array('wps','ppt','dot','xls','doc','docx'))&&$fileType=$ext;
		}
	...
	static function file_down($file_name, $file_path) {
		...
		$downfilename = str_replace(' ','%20',urlencode($file_name));
 ...
 */
 function getdirsize($dir)
	...
	public static function del_file($filepath) {
		if (db_factory::get_count ( sprintf ( " select count(file_id) from %switkey_file where save_name='%s'", TABLEPRE, $filepath ) )) {
			if (is_file ( $filepath )) {
			...
			if (! empty ( $files ) && is_array ( $files )) {
				...
				switch($output){
					case "normal":
						...
						break;
					case "json":
						echo kekezu::json_encode_k ( array ('err' => $err));die ();
						break;
				}
			}
		}
