<?php
define("Msg_WebServerIP", "218.204.253.79");//接收信息的服务器IP
define("Msg_GatewayServer", "211.147.224.154:13013");//发送信息的服务器，网关服务器
...
function Msg_PostSingle($userName, $password, $to, $text, $subId)
{
	...
	if (_Msg_strlen($text) > $maxMessageTextLen)	//判断不确切,因为实际的长度必须减去后缀
		return -5;
//	if (strlen($text) > $maxMessageTextLen)	//判断不确切,因为实际的长度必须减去后缀
//		return -5;
...
define("POST_STRING", "http://%s/cgi-bin/sendsms?".
		"username=%s&password=%s&to=%s&text=%s&subid=%s&msgtype=1");
function _Msg_FinalPostMessage($userName, $password, $to, $text, $subId)
{
	$text = kekezu::utftogbk($text);
