...
class baidu_oauth_client_class extends base_client_class {
	/**
	 * URL prefix definition of Baidu OpenAPI interfaces.
	 */
	public static $BD_OPENAPI_URL_PREFIXS = array ('public' => 'http://openapi.baidu.com/public/2.0/', 'https' => 'https://openapi.baidu.com/rest/2.0/', 'http' => 'http://openapi.baidu.com/rest/2.0/' );
	
	/**
	 * Endpoints definition of Baidu OAuth2.0.
	 */
	public static $BD_OAUTH2_ENDPOINTS = array ('authorize' => 'https://openapi.baidu.com/oauth/2.0/authorize', 'token' => 'https://openapi.baidu.com/oauth/2.0/token', 'logout' => 'https://openapi.baidu.com/connect/2.0/logout' );
	 ...
	 */
	protected $curlOpts = array (CURLOPT_CONNECTTIMEOUT => 10, CURLOPT_TIMEOUT => 30, CURLOPT_USERAGENT => 'baidu-restclient-php-2.0', CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1, CURLOPT_RETURNTRANSFER => true, CURLOPT_HEADER => false, CURLOPT_FOLLOWLOCATION => false );
	...
	function clear_access_token() {
		unset ( $_SESSION ['auth_baidu'] );
		return true;
	}
	/**
	 * 
	 */
	public function post_wb($msg, $img){
	 ...
	 */
	public function get_auth_url($callback) {
		$callback = '/slogin/callback.asp?type=baidu&callback=1';//测试用
		$params = array ('response_type' => 'token', 'client_id' => $this->apiKey, 'redirect_uri' => $callback, 'scope' => '', 'state' => $this->state, 'display' => 'page' );
	...
	public function getAuthorizeUrl($response_type = 'code', $redirect_uri = '', $scope = '', $display = 'popup', $state = '') {
		$params = array ('client_id' => $this->apiKey, 'response_type' => $response_type, 'redirect_uri' => $redirect_uri ? $redirect_uri : $this->getCurrentUrl (), 'scope' => $scope, 'state' => $state ? $state : $this->state, 'display' => $display );
		return self::$BD_OAUTH2_ENDPOINTS ['authorize'] . '?' . http_build_query ( $params, '', '&' );
	}
	...
	public function getAccessTokenByPasswordCredentials($username, $password, $scope = '') {
		$params = array ('grant_type' => 'password', 'username' => $username, 'password' => $password, 'client_id' => $this->apiKey, 'client_secret' => $this->apiSecret, 'scope' => $scope );
	...
	public function getAccessTokenByClientCredentials($scope = '') {
		$params = array ('grant_type' => 'client_credentials', 'client_id' => $this->apiKey, 'client_secret' => $this->apiSecret, 'scope' => $scope );
		return $this->makeAccessTokenRequest ( $params );
	}
	...
	public function getAccessTokenByRefreshToken($refresh_token, $scope = '') {
		$params = array ('grant_type' => 'refresh_token', 'refresh_token' => $refresh_token, 'client_id' => $this->apiKey, 'client_secret' => $this->apiSecret, 'scope' => $scope );
		return $this->makeAccessTokenRequest ( $params );
	}
	...
	public function makeHttpsApiCall($method, $params) {
		...
		$params = array_merge ( array ('access_token' => $this->get_access_token (), 'format' => $this->format ), $params );
	...
	public function makeHttpApiCall($method, $params) {
		...
		$params = array_merge ( array ('session_key' => $this->getSessionKey (), 'timestamp' => date ( 'Y-m-d H:i:s', time () ), 'format' => $this->format ), $params );
