...
SWFUpload.Console.writeLine = function(g) {
	...
	try {
		...
		showDialog("Exception: " + h.name + " Message: " + h.message,'alert',"温馨提示");
							...
							if (e[0] == "*") {
								...
								c("#" + j.queueID)
										...
										.each(
												function() {
													h++;
													var fid = c(this).attr("fid");
													deleteFile(j,i,c(this).index(),fid);
													if (e[1] === true) {
								...
								for ( var m = 0; m < e.length; m++) {
									...
									var fid = q.attr("fid");
										...
										i.cancelUpload(e[m]);
										q.find(".data").removeClass("data").html(" - 已取消");
	...
	var a = {
		...
		onDialogClose : function(d, f, g) {
			...
			if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					showDialog(this.queueData.errorMsg,'alert',"温馨提示");
			...
			for ( var f in this.queueData.files) {
				...
				if (e.uploaded != true && e.name == g.name) {
					var d = showDialog('文件 "'
							+ g.name
							+ '"的文件已在上传队列中.您希望替换吗?',
							'confirm','温馨提示',function(){
								return true;
							});
						/*confirm('The file named "'
							+ g.name
					...
					case SWFUpload.FILE_STATUS.COMPLETE:
						setTimeout(function() {
							if (c("#" + f.id)) {
								...
								g.hide==true&&c("#" + f.id).fadeOut(500, function() {
									c(this).remove();
								});
							}
		...
		onUploadError : function(e, i, h) {
			...
			/*case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				showDialog("The upload limit has been reached (" + h + ").",'alert',"warnings");
