...
if(check_user_login()){
	if (uid == to_uid) {
		...
		var url = encodeURI('/index.php?do=ajax&view=message&op=send&to_uid='+ to_uid+'&to_username='+to_username);
		showWindow('message',url);return false;
}
}
/**
...
function showDialog(msg, mode, t, func, cover, funccancel,auto) {
...
function ajaxpage(ajaxDom,loadUrl,loadPage){
	...
	if(pageDom.length==0&&loadPage>1){
		showDom.load(loadUrl+'&inajax=1 #'+ajaxDom).show();
		showDom.siblings().hide();
		showDom.before(showDom.clone(true).hide());
		showDom.get(0).setAttribute("id","page"+loadPage);
		$("html,body").animate({scrollTop: $("#page"+loadPage).offset().top});
	}else{
		if(loadPage==1){
			showDom.show().siblings().hide();
			$("html,body").animate({scrollTop: $("#gj_summery").offset().top});
		}else{
...
function findIndusById(id, indusList) {
	var ret;
	_.find(indusList, function(indus) {
	   if(indus.indus_id == id) {
	   		ret = indus;
	   		return true;
	   }
	   if(indus.children) {
	       var tmp = findIndusById(id, indus.children);
	       if(tmp) {
	       		ret = tmp;
	       		return true;
	       }
