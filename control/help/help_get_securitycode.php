...
$page_title="找回安全码".'- '.$_K['html_title'];
...
if (kekezu::submitcheck($formhash)) {
	...
	switch ($accout_type){
		case "email":
			...
			if($auth_arr){
				if($auth_arr['email']==$txt_email){
					...
					keke_shop_class::notify_user($user_info['uid'], $user_info['username'], 'get_password',"密码重置通知",$v_arr);
					//kekezu::show_msg($_lang['friendly_notice'],$r,3,$_lang['your_new_password_in_email']);
					kekezu::echojson($_lang['your_new_securitycode_in_email']."<br />".$txt_email."请注意查收！",1,$user_info);
					die();
				}else{
					$user_info['txtemail']=$txt_email;
					$user_info['authemail']=$auth_arr['email'];
		...
		case "mobile":
			...
			if($mobile_auth){
				if($mobile_auth['mobile']==$txt_mobile){
					$pass_info = reset_set_password($user_info);
					...
					kekezu::echojson($_lang['securitycode_has_send_to_phone'],1,$user_info);
					die();
