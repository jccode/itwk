...
if(isset($p)){
	...
	switch ($p){
		case 'others':
			...
			break;
		case '1':
			$where.=sprintf ( " and  ( uid  in (select DISTINCT(uid) from ".TABLEPRE."witkey_member_skill where skill_id in (select indus_id from keke_witkey_industry where indus_pid in ('1858','1867','1868','1870','1879'))))" );
			break;	
		case '2':
			$where.=sprintf ( " and  ( uid  in (select DISTINCT(uid) from ".TABLEPRE."witkey_member_skill where skill_id in (select indus_id from keke_witkey_industry where indus_pid in ('1871','1872','1873'))))" );
			break;	
		case '3':
			$where.=sprintf ( " and  ( uid  in (select DISTINCT(uid) from ".TABLEPRE."witkey_member_skill where skill_id in (select indus_id from keke_witkey_industry where indus_pid in ('1874','1875'))))" );
			break;	
		case '4':
			$where.=sprintf ( " and  ( uid  in (select DISTINCT(uid) from ".TABLEPRE."witkey_member_skill where skill_id in (select indus_id from keke_witkey_industry where indus_pid in ('1880','1881','1882','1883','1884','1877','1878','1879'))))" );
			break;	
		case '5':
			$where.=sprintf ( " and  ( uid  in (select DISTINCT(uid) from ".TABLEPRE."witkey_member_skill where skill_id in (select indus_id from keke_witkey_industry where indus_pid in ('1891','1893','1886','1887','1888','1889','1890','1885'))))" );
			break;			
		default:
		    $where.=sprintf ( " and  ( uid  in (select DISTINCT(uid) from ".TABLEPRE."witkey_member_skill where skill_id in (select indus_id from keke_witkey_industry where indus_pid=%d)))", '1858' );
	        break;
	}
...
if(!isset($p)) {
	$p = array_keys($indus_p_arr)[0];
}
$indus_info = $kekezu->findIndusById($p);
$pids = $kekezu->get_indus_ids_below_except_leaf($indus_info);
$where.=sprintf ( " and  ( uid  in (select DISTINCT(uid) from ".TABLEPRE."witkey_member_skill where skill_id in (select indus_id from keke_witkey_industry where indus_pid in (%s))))", join(", ", $pids) );
