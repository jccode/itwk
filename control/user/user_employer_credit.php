...
switch ($opp) {
	case "grow" :
		...
		/**筛选条件**/;	
		/***分页统计*/
		$count = intval(db_factory::get_count(' select count(mark_id) m from '.TABLEPRE.'witkey_mark where uid='.$uid.' and mark_type=1 ',0,'m',600));
		...
		$sql = ' select a.*,b.task_title from '.TABLEPRE.'witkey_mark a left join '.
				TABLEPRE.'witkey_task b on a.origin_id=b.task_id where a.uid='.$uid.' and mark_type=1 order by mark_time desc '.$pages['where'];
	...
	case "mark" :
		...
		if($role_type=='1'){
			$wc.=" uid='{$uid}' and mark_type=1";
			$wq.=" a.uid='{$uid}' and mark_type=1";
		}else{
			$wc.=" by_uid='{$uid}' and mark_type=1";
			$wq.=" a.by_uid='{$uid}' and mark_type=1";//角色类型为1=>我是被评价者uid  2=>我是评价者by_uid
		}
