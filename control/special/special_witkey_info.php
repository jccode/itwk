...
if(isset($art_id) && intval($art_id)){
	...
	//跳转处理
	if(!$art_info){
		kekezu::show_msg('网页错误','/index.php?do=article&art_cat_id=594',2,'您查看的文章不存在，您可以返回列表查看其它文章。','warning');
	}
	...
	kekezu::show_msg("操作提示",$_K['siteurl']."/index.php?do=special&view=witkey_list",3,"文章参数错误，请返回列表页重试！","warning");
}
