...
if($ac == 'tiqu' && $t_uid){
	...
	if(in_array($admin_info['uid'],array(1,41645))){
		...
		$space_obj->setTrack_username( $yewuyuan[$to_track_uid] );
 ...
 * $time = $stat = array();
			 ...
			 //上周		
			$time['last_week_start'] = mktime(0, 0 , 0,date("m"),date("d")-date("w")+1-7,date("Y"));  
			$time['last_week_end'] = mktime(23,59,59,date("m"),date("d")-date("w")+7-7,date("Y")); 
			$stat['last_week'] = db_factory::get_count(" select count(*) from " . TABLEPRE . "witkey_space where track_uid ='$w[track_uid]' and last_track_time > $time[last_week_start] and last_track_time < $time[last_week_end]");
			
			 //本周 
			$time['this_week_start'] = mktime(0, 0 , 0,date("m"),date("d")-date("w")+1,date("Y")); 
			$time['this_week_end'] = mktime(23,59,59,date("m"),date("d")-date("w")+7,date("Y")); 
			$stat['this_week'] = db_factory::get_count(" select count(*) from " . TABLEPRE . "witkey_space where track_uid ='$w[track_uid]' and last_track_time > $time[this_week_start] and last_track_time < $time[this_week_end]");
			
			 //上月
			$time['last_month_start'] = mktime(0, 0 , 0,date("m")-1,1,date("Y")); 
			$time['last_month_end'] = mktime(23,59,59,date("m") ,0,date("Y")); 
			$stat['last_month'] = db_factory::get_count(" select count(*) from " . TABLEPRE . "witkey_space where track_uid ='$w[track_uid]' and last_track_time > $time[last_month_start] and last_track_time < $time[last_month_end]");
			
			 //本月
			$time['this_month_start'] = mktime(0, 0 , 0,date("m"),1,date("Y")); 
			$time['this_month_end'] = mktime(23,59,59,date("m"),date("t"),date("Y")); 
			$stat['this_month'] = db_factory::get_count(" select count(*) from " . TABLEPRE . "witkey_space where track_uid ='$w[track_uid]' and last_track_time > $time[this_month_start] and last_track_time < $time[this_month_end]");
			
			 //本季度
			$getMonthDays = date("t",mktime(0, 0 , 0,date('n')+(date('n')-1)%3,1,date("Y"))); //本季度未最后一月天数
			$time['this_quarter_start'] = mktime(0, 0, 0,date('n')-(date('n')-1)%3,1,date('Y')); 
			$time['this_quarter_end'] = mktime(23,59,59,date('n')+(date('n')-1)%3,$getMonthDays,date('Y')); 
			$stat['this_quarter'] = db_factory::get_count(" select count(*) from " . TABLEPRE . "witkey_space where track_uid ='$w[track_uid]' and last_track_time > $time[this_quarter_start] and last_track_time < $time[this_quarter_end]");
 * */
