...
$url = "index.php?do=$do&view=$view&w[comment_id]=$w[comment_id]&w[art_title]=$w[art_title]&w[username]=$w[username]&w[content]=$w[content]&w[page_size]=$w[page_size]&page=$page";
...
if (isset ( $ac ) && $comment_id) { //删除单条留言
	...
	$res and kekezu::admin_show_msg ( $_lang['mulit_operate_delete_success'], $url,3,'','success' ) or kekezu::admin_show_msg ( $_lang['mulit_operate_delete_fail'], $url,3,'','warning' );
	 ...
	 //条件
	$where = " where c.obj_type = 'article' and c.p_id = 0 ";   //默认查询条件
	...
	is_array($w['ord']) and $where .= ' order by c.'.$w['ord'][0].' '.$w['ord'][1] or $where .= "order by c.comment_id desc ";
	 ...
	 //行数
	$sql_count = "select count(*) from ".TABLEPRE."witkey_comment as c left join ".TABLEPRE."witkey_article as a on c.obj_id = a.art_id ";
	$sql_count .= $where; 
	...
	$sql = "select c.*,a.*,c.username as username,c.content as content from ".TABLEPRE."witkey_comment as c left join ".TABLEPRE."witkey_article as a on c.obj_id = a.art_id ";
