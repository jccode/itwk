...
elseif (isset ( $p )) {
	$pids_in = join(", ", $pids);
	$where .= sprintf ( " and  ( indus_pid in ( %s ) or indus_id in (%s) )", $pids_in, $pids_in );
...
$task_new4 = db_factory::query ( sprintf ( "select task_id,task_title,task_cash,view_num,work_num,task_cash_coverage from %switkey_task  where (task_status='2' or task_status='3') and is_recommend='1' order by start_time desc limit 0,30", TABLEPRE ), true, 86400 );
...
if(in_array($tid,$d6) or in_array($tid1,$d6)){
$ab6=' class="menuoverff"';
$d=array(1961);
}
*/
if(!$tid){
...
//     $nav = array($indus_vals[0]);
	$nav = array();
	$l1tid = array_values($indus_map)[0]["indus_id"];
} else {
