...
$page_size = 10;
$url=$_K['siteurl']."/index.php?do=shop&sid=".$shop_info['shop_id']."&view=match_list&page=".$page."&op=".$op;
...
if(isset($op) && $shop_info['city_match']==1){
	$where = " where model_id=4 and ifnull(task_type,0)!=3 and indus_id in (".$shop_info['skill_ids'].") ";
	$shop_info['global_match'] or $where.=" and FIND_IN_SET('".$area."',city)>0 ";
	$w     = ' and a.uid != '.$shop_info['uid'].' and a.task_status = 2 and sub_time>'.time(); //修改投稿时间判断
	
	//=---------------------统计------------------=//
	//所有有任务
	$s_csql = " select count(a.task_id) c from ".TABLEPRE."witkey_task a ".$where
				.$w ;
	$s_c    = intval(db_factory::get_count($s_csql,0,'c',600));
	//未报价
	...
	//已中标
	$b_csql = sprintf("select count(a.task_id) c from %switkey_task a left join ".TABLEPRE."witkey_task_work b on a.task_id=b.task_id and b.work_status between 1 and 12",TABLEPRE).$where
	...
	switch ($op){
		...
		case 2://已报价
			...
			break;
		case 3://中标
			$sql = sprintf("select a.* from %switkey_task a left join ".TABLEPRE."witkey_task_work b on a.task_id = b.task_id and b.work_status between 1 and 12",TABLEPRE).$where.' and a.w_uid='.$shop_info['uid'];
			$count   = $b_c;
			break;
	}
