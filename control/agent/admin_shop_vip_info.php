...
if($shop_id&&$_SESSION['brandType']){
	...
	if(!$auth){//无权限则跳出
		kekezu::admin_show_msg ( $_lang ['no_auth'], $_K['siteurl']."/control/agent/index.php?do=shop&view=vip_list", 3, '', 'error' );
		die();
	}
}
...
//保存编辑
if($sbt_edit){
	kekezu::admin_system_log($_lang['edit_user_shop_info']."[".$shop_info['shop_id']."]");
