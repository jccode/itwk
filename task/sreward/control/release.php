...
$model_info ['config'] and $sreward_config = unserialize ( $model_info ['config'] ); //任务配置
//防错验证
if (! $task_info) {
	kekezu::keke_show_msg ( $_K ['siteurl'] . "/index.php?do=release", '该任务不存在或者参数错误，请重新发布', 'error' );
}
if ($r_step == 'step2') {
	if (intval ( $txt_task_period ) < 1 || intval ( $txt_task_cash ) < $sreward_config['min_cash']) { //时间为负。空,金额为负数。防止页面修改注入
...
switch ($r_step) { //任务发布步骤
	case "step2" :
		...
		$end_time = $sub_time + ($sreward_config ['choose_time'] * 24 * 3600);
		...
		if ($ac == 'save') {
			...
			header ( "location:" . $_K ['siteurl'] . "/index.php?do=release&pub_mode=$pub_mode&t_id=$t_id&task_id=$task_id&model_id={$model_id}&r_step=step3" );
		}
		die ();
		
		break;
	case "step3" :
		$payitem_arr = keke_payitem_class::get_payitem_info ( "employer", $model_info ['model_code'] ); //雇主
		$pay_item = $task_info ['pay_item'];
		
		//当前用户是否是vipkeke_witkey_space
		$user_space_info = kekezu::get_user_info ( $_SESSION ['uid'] ); //获取用户信息
		

		//$pay_item = implements(",",$pay_item );
		//var_dump($pay_item);exit;
		...
		if (kekezu::submitcheck ( $formhash )) {
			...
			$task_info ['att_credit'] != $item_total_credit and $update_arr ['att_credit'] = $item_total_credit; //增值论坛积分
