...
if($task_info['task_type']==3){
	...
	if ($hire_work['uid']!=$uid&&$task_info['uid']!=$uid&&$uid!=ADMIN_UID){
		kekezu::show_msg("无权限浏览","index.php");//只有双方可以浏览
	}
	$view or $view= 'work';
...
//访问状态判断
if(in_array($task_info['task_status'],array(-1,0,1,10))&&$uid!=$task_info['uid']&&$uid!=ADMIN_UID&&$userinfo['user_group']!=7&&$uid!=$hire_work['uid']){
...
//		break;
	...
	case "confirm_pay":
		...
		$task_obj->create_mark_log(
					array('uid'=>$task_info['uid'],'username'=>$task_info['username'],'cash'=>$cash),
					array('uid'=>$bid_info['uid'],'username'=>$bid_info['username'],'cash'=>$cash),
					$bid_info['work_id']);
