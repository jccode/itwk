...
$model_info ['config'] and $preward_config = unserialize ( $model_info ['config'] ); //任务配置
...
if ($r_step == 'step2') {
	if (intval ( $txt_task_period ) < 1 || intval ( $txt_task_cash ) < $preward_config['min_cash'] || ($txt_work_count * $txt_single_price) != $txt_task_cash) { //时间为负。空,金额为负数。防止页面修改注入
...
switch ($r_step) { //任务发布步骤
	case "step2" :
		...
		$end_time = $sub_time + ($preward_config ['choose_time'] * 24 * 3600);
		...
		$task_obj->setWork_count ( $txt_work_count );
		$task_obj->setSingle_cash ( $txt_single_price );
