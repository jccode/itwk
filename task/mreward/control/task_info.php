...
if($task_info['exec_time']&&$task_info['exec_time']<time()&&$task_info['task_union']!=2){
	$time_obj = new mreward_time_class(); 
...
switch ($op) {
	...
	case "edit" : //任务修改
		...
		if($sbt_edit){
			...
			if($t=='load'){//异步加载本任务模型的编辑处理页面代码
				...
				require keke_tpl_class::template('task/mreward/tpl/'.$_K['template'].'/task_edit');die();
	...
	case "work_remark" : //备选稿件
		...
		kekezu::keke_show_msg ( $url, '稿件设置成功', "", 'json' );
...
//中标稿件的源文件获取
if($search_condit[1]||$search_condit[2]||$search_condit[3]||$search_condit[4]||$search_condit[5]){
	$temp = db_factory::query("select a.* from ".TABLEPRE."witkey_file a inner join ".TABLEPRE."witkey_task_work b on a.work_id = b.work_id and b.task_id = '$task_id' and b.work_status in (1,2,3,4,5) and a.obj_type='worksource'");
...
if($view=='work'){
	$work_info = db_factory::get_one(" select a.*,b.isvip,b.seller_credit,b.seller_good_num,b.residency,b.seller_total_num,b.w_level,b.auth_bank,b.auth_realname,b.auth_email,b.auth_mobile from " . TABLEPRE . "witkey_task_work a left join " . TABLEPRE . "witkey_space b on a.uid=b.uid where a.work_id='$work_id'");
