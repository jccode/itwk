...
$data = array();
 $data['pub_num'] = intval(db_factory::get_count("select count(task_id) from ".TABLEPRE."witkey_task where task_status>=-1 and model_id >0 and uid =".$uid));//发布
 $data['join_num']= intval(db_factory::get_count("select count(work_id) from ".TABLEPRE."witkey_task_work where  uid =".$uid));;//参加
 ...
 $m_get  = db_factory::get_count(" select count(msg_id) c from ".TABLEPRE."witkey_msg where to_uid='{$uid}' and msg_pid=0 and view_status=0 and msg_status!=1");
